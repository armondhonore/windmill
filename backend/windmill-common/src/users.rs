...
/// True if a run-as identity resolves to a reserved superadmin sentinel.
/// `fetch_authed_from_permissioned_as` grants superadmin when *either* the
/// `permissioned_as` (matched against `SUPERADMIN_SYNC_EMAIL`) or the `email`
/// matches a sentinel, so both must be checked. This is the only identity that
/// escalates an app to instance-superadmin, so it is the single thing a
/// preserved app policy must never carry — other identities (stale, raw-email,
/// or null-email authors) are preserved as-is so legacy apps still redeploy.
pub fn is_reserved_superadmin_identity(permissioned_as: &str, email: Option<&str>) -> bool {
    is_reserved_superadmin_email(permissioned_as) || email.is_some_and(is_reserved_superadmin_email)
}
